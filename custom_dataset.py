...
class CustomDetection(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_path = image_dir + anno_path[len(self.label_dir):-4] 

        bboxs, cls_ids = self.get_annotations(anno_path)
        image = cv2.imread(img_path, cv2.IMREAD_COLOR)

        if self.dbtype == 'train':
            image, bboxs, cls_ids = preproc_for_train(image, boxes, labels, opt.min_size, opt.mean)
    ...
    def get_annotations(path):
        ...
        while line:
            ...
            xmin = int((label[1] * 800) - (label[3] * 800 / 2))
            ymin = int((label[2] * 240) - (label[4] * 240 / 2))

            xmax = int((label[1] * 800) + (label[3] * 800 / 2))
            ymax = int((label[2] * 240) + (label[4] * 240 / 2))
